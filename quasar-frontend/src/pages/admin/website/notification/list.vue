<template>
  <q-page class="container-lg q-my-md" padding>
    <div class="zcard">
      <p class="ztitle">Government Notifications</p>
...
import Edit from "pages/admin/website/notification/Edit.vue";
...
export default {
  ...
  setup(props, context) {
    ...
    const handleDownload = (item) => {
      api.get(`web/notification/${item.id}/download`).then((res) => {
        const { data } = res.data;
    ...
    const handleEdit = (item) => {
      localData.notification = Object.assign(localData.notification, item);

      // return console.log('item',localData.notification);
    ...
    const handleSearch = (e) => {
      ...
      if (e.keyCode === 13) fetchNotifications(1);
    };
    ...
    const fetchNotifications = (page) => {
      api
        .get(`web/notification?page=${page}`, {
