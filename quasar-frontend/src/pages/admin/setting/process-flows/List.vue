    ...
    <div>
      <div class="row q-col-gutter-md">
        <div class="flex justify-between flex-inline col-12">
          <q-btn
            :to="{ name: 'process-flows:create' }"
            outline
            label="New process flow"
            color="primary"
          />
          <q-input
      ...
      <q-dialog v-model="localData.openFlow">
        <Detail
          :application_name="localData.selectedFlow?.title"
...
export default {
  ...
  setup(props, context) {
    ...
    const deleteFlow = (id) => {
      api
        .delete(`application-profiles/process-flows/${id}`)
        .then((res) => {
    ...
    const fetchStaff = (page) => {
      api
        .get(`application-profiles/flows?page=${page}`, {
    ...
    onMounted(() => fetchStaff(localData.listData.current_page));
    return {
      updatePagination,
      pageCount: computed(() =>
